...
public class GrinderRecipeBuilder {
  ...
  protected GrinderRecipeBuilder(IItemProvider result, ITag.INamedTag<Item> tag, float xp, int cookTime, int count, boolean staticOutput) {
  ...
  protected GrinderRecipeBuilder(ITag.INamedTag<Item> result, IItemProvider input, float xp, int cookTime, int count, boolean staticOutput) {
  ...
  public static GrinderRecipeBuilder builder (IItemProvider result, ITag.INamedTag<Item> tag) {
  ...
  public static GrinderRecipeBuilder builder (IItemProvider result, ITag.INamedTag<Item> tag, int count) {
  ...
  public static GrinderRecipeBuilder builder (IItemProvider result, ITag.INamedTag<Item> tag, int count, boolean staticOutput) {
    return new GrinderRecipeBuilder(result, tag, 0.125f, 100, count, staticOutput);
  }

  public static GrinderRecipeBuilder builder (ITag.INamedTag<Item> result, IItemProvider tag, int count, boolean staticOutput) {
    return new GrinderRecipeBuilder(result, tag, 0.125f, 100, count, staticOutput);
  }
  ...
  private void addOre(ITag.INamedTag<Item> ore) {
    this.advancementBuilder.withCriterion("has_ore", new InventoryChangeTrigger.Instance(EntityPredicate.AndPredicate.ANY_AND, MinMaxBounds.IntBound.UNBOUNDED, MinMaxBounds.IntBound.UNBOUNDED, MinMaxBounds.IntBound.UNBOUNDED, new ItemPredicate[]{ItemPredicate.Builder.create().tag(ore).build()}));
  }

  private void addInput (Item input) {
    this.advancementBuilder.withCriterion("has_input", new InventoryChangeTrigger.Instance(EntityPredicate.AndPredicate.ANY_AND, MinMaxBounds.IntBound.UNBOUNDED, MinMaxBounds.IntBound.UNBOUNDED, MinMaxBounds.IntBound.UNBOUNDED, new ItemPredicate[]{ItemPredicate.Builder.create().item(input).build()}));
  }

  public void build(Consumer<IFinishedRecipe> consumer) {
    this.build(consumer, new ResourceLocation(Grinder.MODID, "grinding/" + ForgeRegistries.ITEMS.getKey(this.result).getPath()));
  }
  ...
  public void build(Consumer<IFinishedRecipe> consumer, String name) {
    ...
    ResourceLocation recipeName = new ResourceLocation(Grinder.MODID, "grinding/" + name);
  ...
  public void build(Consumer<IFinishedRecipe> consumer, ResourceLocation resource) {
    ...
    if (tag != null) {
      ConditionalRecipe.builder().addCondition(new NotCondition(new TagEmptyCondition(tag.getName()))).addRecipe(new GrinderRecipeBuilder.Result(resource, this.group == null ? "" : this.group, this.ingredient, this.result, this.experience, this.cookingTime, this.advancementBuilder, new ResourceLocation(resource.getNamespace(), "recipes/" + this.result.getGroup().getPath() + "/" + resource.getPath()), resultIngredient, count, staticOutput)).setAdvancement(new ResourceLocation(Grinder.MODID, "recipes/" + resource.getPath()), ConditionalAdvancement.builder().addCondition(new NotCondition(new TagEmptyCondition(tag.getName()))).addAdvancement(this.advancementBuilder)).build(consumer, resource);
    } else {
