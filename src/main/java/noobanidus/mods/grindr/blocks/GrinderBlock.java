...
public class GrinderBlock extends AbstractFurnaceBlock {
  ...
  public static BooleanProperty HAS_GRINDSTONE = BooleanProperty.create("has_grindstone");
  ...
  public GrinderBlock(Properties p_i48440_1_) {
    super(p_i48440_1_);
    setDefaultState(this.getDefaultState().with(FACING, Direction.NORTH).with(ACTIVE, false).with(GRINDSTONE, GrindstoneType.EMPTY).with(HAS_GRINDSTONE, false));
  }
  ...
  protected void interactWith(World world, BlockPos blockPos, PlayerEntity playerEntity) {
    ...
    if (te instanceof GrinderTile) {
      ...
      if (playerEntity.getHeldItemMainhand().getItem() instanceof GrindstoneItem) {
        if (gte.getGrindstone() == GrindstoneType.EMPTY) {
  ...
  public void onReplaced(BlockState state, World worldIn, BlockPos pos, BlockState newState, boolean isMoving) {
    if (state.getBlock() != newState.getBlock()) {
      ...
      if (tileentity instanceof GrinderTile) {
        GrinderTile te = (GrinderTile) tileentity;
        GrindstoneType type = te.getGrindstone();
        if (type != GrindstoneType.EMPTY) {
          RegistryEntry<GrindstoneItem> item = ModItems.GRINDSTONE_MAP.get(type);
          if (item != null) {
  ...
  public VoxelShape getShape(BlockState state, IBlockReader p_220053_2_, BlockPos p_220053_3_, ISelectionContext
      p_220053_4_) {
    return EMPTY_VOXEL;
  }
  ...
  protected void fillStateContainer(StateContainer.Builder<Block, BlockState> builder) {
    builder.add(GRINDSTONE, FACING, ACTIVE, HAS_GRINDSTONE);
  }
