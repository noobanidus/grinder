...
public class TagCookingRecipeBuilder<T extends AbstractCookingRecipe> {
  protected final Tag<Item> result;
  ...
  protected TagCookingRecipeBuilder(IRecipeSerializer<?> serializer, Tag<Item> result, Ingredient ingredient, float xp, int cookTime) {
    this.result = result;
  ...
  public static TagCookingRecipeBuilder<BlastingRecipe> blastingRecipe (Ingredient ingredient, Tag<Item> result, float xp, int cookTime) {
    return new TagCookingRecipeBuilder<>(ModRecipes.TAG_BLASTING_SERIALIZER.get(), result, ingredient, xp, cookTime);
  }

  public static TagCookingRecipeBuilder<FurnaceRecipe> furnaceRecipe (Ingredient ingredient, Tag<Item> result, float xp, int cookTime) {
    return new TagCookingRecipeBuilder<>(ModRecipes.TAG_FURNACE_SERIALIZER.get(), result, ingredient, xp, cookTime);
  }

  public static void ingot (IItemProvider input, Tag<Item> output, RegistrateRecipeProvider consumer) {
    Ingredient inputIngredient = Ingredient.fromItems(input.asItem());
    String outputPath = output.getId().getPath().replace("ingots/", "");
    furnaceRecipe(inputIngredient, output, 0.125f, 200).addCriterion("has_dust", consumer.hasItem(input)).build(consumer, new ResourceLocation(Grindr.MODID, "smelting/" + outputPath + "_by_smelting"));
    blastingRecipe(inputIngredient, output, 0.125f, 100).addCriterion("has_dust", consumer.hasItem(input)).build(consumer, new ResourceLocation(Grindr.MODID, "blasting/" + outputPath + "_by_blasting"));
  }
  ...
  public void build(Consumer<IFinishedRecipe> consumer, ResourceLocation resource) {
    ...
    ConditionalRecipe.builder().addCondition(new NotCondition(new TagEmptyCondition(result.getId()))).addRecipe(new TagCookingRecipeBuilder.Result(resource, this.group == null ? "" : this.group, this.ingredient, Ingredient.fromTag(this.result), this.experience, this.cookingTime, this.advancementBuilder, new ResourceLocation(resource.getNamespace(), "recipes/" + resource.getNamespace() + "/" + resource.getPath()), getSerializer())).setAdvancement(new ResourceLocation(Grindr.MODID, "recipes/" + resource.getPath()), ConditionalAdvancement.builder().addCondition(new NotCondition(new TagEmptyCondition(result.getId()))).addAdvancement(this.advancementBuilder)).build(consumer, resource);
  }
